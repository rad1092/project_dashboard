...
ALERT_STRIP_SURFACE = "rgba(255, 250, 242, 0.84)"
ALERT_STRIP_BADGE_BG = "rgba(47, 143, 131, 0.14)"
...
def _mix_hex_color(color: str, target: str, target_weight: float) -> str:
    ...
    for index in range(0, 6, 2):
        source_channel = int(source[index : index + 2], 16)
        destination_channel = int(destination[index : index + 2], 16)
        mixed_channel = round((source_channel * (1 - weight)) + (destination_channel * weight))
        channels.append(f"{mixed_channel:02x}")
    return f"#{''.join(channels)}"


def _is_green_family_color(color: str) -> bool:
    ...
    *,
    ...
    for index, (label, value) in enumerate(meta_rows):
        ...
        value_style = (
            ...
            f" font-size: {meta_text_font_size};"
            " font-weight: 700;"
            " line-height: 1.15;"
            " min-width: 0;"
            " overflow: hidden;"
...
">
    ...
    note_content = _escape_card_text(note) if note else "&nbsp;"
    note_visibility = "visible" if note else "hidden"
    note_border = f"1px solid {divider_color}" if note else "1px solid transparent"
    note_block = dedent(
        f"""\
...
<div class="pd-current-alert-strip__item" style="
    ...
    border-radius: 999px;
    border: 1px solid {CARD_META_BORDER};
    background: {ALERT_STRIP_SURFACE};
">
    <span style="
...
<div class="pd-current-alert-strip" style="
    ...
    border: 1px solid {CARD_META_BORDER};
    background:
        radial-gradient(circle at top left, rgba(47, 143, 131, 0.16), transparent 36%),
        radial-gradient(circle at bottom right, rgba(212, 183, 137, 0.14), transparent 30%),
        linear-gradient(135deg, {CARD_SURFACE_TOP}, {CARD_SURFACE_BOTTOM});
    box-shadow: 0 24px 80px {CARD_SHADOW};
">
    <div class="pd-current-alert-strip__badge" style="
        ...
        border-radius: 999px;
        background: {ALERT_STRIP_BADGE_BG};
        border: 1px solid {ALERT_STRIP_BADGE_BORDER};
        color: {ALERT_STRIP_BADGE_TEXT};
        font-size: 0.84rem;
        font-weight: 800;
        letter-spacing: -0.01em;
        white-space: nowrap;
